...
轻量模式是 OhMyWu 的桌面交互壳，提供纯 Live2D 桌宠体验。

**窗口分离设计：**

| 窗口 | 类型 | 定位 | 渲染内容 |
|------|------|------|----------|
| Main Window | 标准 BrowserWindow | 主应用入口 | Shell UI + 桌宠按钮 |
| Pet Window | frameless + transparent + alwaysOnTop | 桌面悬浮 | 仅 Live2DPet 组件 |

**窗口检测机制：**

```ts
// Pet Window 通过 URL 参数 ?pet=1 自我识别
...
```ts
// preload.ts 暴露
window.ohmywu.pet.enter()   // Main → Main隐藏 + Pet Window创建
window.ohmywu.pet.exit()    // Pet → Pet Window关闭 + Main恢复
```
...
| 文件 | 职责 |
|------|------|
| `desktop/electron/main.ts` | Pet Window 创建、Tray 菜单、IPC handlers |
| `desktop/electron/preload.ts` | pet.enter/exit contextBridge |
| `apps/web/src/App.vue` | 窗口模式检测 + 条件渲染 |
| `apps/web/src/components/Live2DPet.vue` | Live2D 渲染 + 交互 |
| `apps/web/public/live2d/` | Live2D JS 库 + 模型文件 |
