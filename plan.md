...
## 0.15 当前最重要补充：上下文管理 + 显式 @action / @记忆

在当前阶段，这两块的优先级应该进一步前提，甚至要高于多 Agent 扩展本身。

原因很直接：

1. 没有上下文管理，长任务一旦进入多轮工具调用，token 成本和响应质量都会迅速恶化
2. 没有显式上下文入口，用户无法精确告诉系统“这轮必须参考哪条记忆 / 哪个 action”
3. 没有显式 action 触发，Action 目录就只是注册表，而不是实际可操作的工作流入口

因此，下一阶段的第一主线应定义为：

### A. 自动上下文压缩 / 上下文管理

目标不是简单“少传一点消息”，而是建立一个真正可控的 turn context manager。
...
目标是让用户在输入框里直接声明“这轮要用哪个动作、哪条记忆”，而不是完全依赖模型猜。

建议语义：

1. `@action`
   - 例：`@system.skill_to_action`
   - 作用：把该 action 的 blueprint / prompt 注入本轮上下文，并提高其被采用的优先级
   - 不是“立刻执行”，而是“本轮显式激活”

2. `@memory`
   - 例：`@memory:ohmywu-guide`
   - 作用：把指定 wiki 笔记直接 pin 到本轮上下文
   - 优先级高于自动 recall，不参与普通召回竞争
...
4. 后端结构化传输
   - 不要只传纯文本
   - 前端应把 mention 解析成结构化 payload：
     - `mentioned_actions`
     - `mentioned_memories`
   - 后端按结构化字段组装 turn context，而不是再从原文硬解析

### C. 这一块的验收标准

1. 一轮消息里可以显式选择一个或多个 action
2. 一轮消息里可以显式 pin 一个或多个知识库笔记
3. runtime 里能看到本轮上下文构成
4. 长对话不会再简单线性回放全部历史
5. 大工具输出不会直接炸穿上下文
6. 显式 `@记忆` 在超预算时不会被优先裁掉
...
3. 上下文回放成本
   - 不再简单固定“最近 20 条”
   - 要把上下文体积控制成“必要消息 + 显式 mentions + 压缩摘要 + 大结果句柄”
